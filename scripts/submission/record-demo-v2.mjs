...
const RAW_VIDEO = join(OUTPUT_DIR, process.env.WALKTHROUGH_FILE ?? 'walkthrough-jury-v2.webm')
const DURATION_MS = Number(process.env.DEMO_DURATION_MS ?? '142000')

if (!Number.isFinite(DURATION_MS) || DURATION_MS < 90_000 || DURATION_MS > 179_000) {
  throw new Error('DEMO_DURATION_MS must be between 90000 and 179000.')
}
...
try {
  ...
  await until(46_000)
  await pointTo(page.getByText(/CACHED REAL DATA/i).first())
  await until(51_000)
